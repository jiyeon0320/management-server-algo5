...
router.post('/view-grid', function (req, res) {
    ...
    let querystr = 'CALL study.sp_select_schedule(?, ?, ?)';
    let queryparams = [req.body.from_date, req.body.to_date, req.body.grade];
    let rdata = {};
...
router.post('/update-grid', function (req, res) {
    ...
    let iquery =
        'INSERT INTO LIST_DAILY_ALGORITHM_TEST (study_date, grade, original_id, trim_date) VALUES (?,?,?,?)';
    let uquery =
        'UPDATE LIST_DAILY_ALGORITHM_TEST SET study_date=?, grade=?, original_id=?, trim_date=? WHERE dailyno=?';
